...
def int2roman ( num ):
    ...
    if (num >= 4000000) or (num < 1):
        ...
        cnum   = str(num)                              # Convert num to string.
        strlen = len(cnum)-1                           # Find out how many digits are in 
                                                       # the input number (magnitude).
        roman = ""                                     # Empty string.

        # The loop is executed at most 7 times, and 
        # it uses no division or multiplication.
        for c in cnum:                                 # Loop forward  (0,1,...,len-1).
            dig = int (c)                              # dig between [0,9]. int(c) == ord(c) - ord("0")
            roman += symbols [ strlen ] [ dig ]        # Loop backward (len-1,len-2,len-3,...,0).
            strlen -= 1
